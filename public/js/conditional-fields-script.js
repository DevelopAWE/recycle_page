...
(function($) {
  ...
  $(document).ready(function() {
    function display_fields(unit_tag, wpcf7cf_conditions, wpcf7cf_settings) {
      ...
      if (
        ...
        for (var i = 0; i < signatures.length; i++) {
          if (signatures[i].canvas.width == 0) {
            jQuery(".wpcf7-form-control-signature-body>canvas")
              .eq(i)
              .attr(
                "width",
                jQuery(".wpcf7-form-control-signature-wrap").width()
              );
            jQuery(".wpcf7-form-control-signature-body>canvas")
              .eq(i)
              .attr(
                "height",
                jQuery(".wpcf7-form-control-signature-wrap").height()
              );
      ...
      $("#" + unit_tag + " [data-class='wpcf7cf_group']").addClass(
        "wpcf7cf-hidden"
      );
      ...
      for (var i = 0; i < wpcf7cf_conditions.length; i++) {
        ...
        $field = $("#" + unit_tag + ' [name="' + condition.if_field + '"]')
          .length
          ? $("#" + unit_tag + ' [name="' + condition.if_field + '"]')
          : $("#" + unit_tag + ' [name="' + condition.if_field + '[]"]');
        ...
        if ($field.length == 1) {
          ...
          if ($field.is("select")) {
            ...
            $field.find("option:selected").each(function() {
              ...
              if (
                (condition.operator == "equals" &&
                  $option.val() == condition.if_value) ||
                (condition.operator == "equals (regex)" &&
                  regex_patt.test($option.val()))
              ) {
                show = true;
              } else if (
                (condition.operator == "not equals" &&
                  $option.val() == condition.if_value) ||
                (condition.operator == "not equals (regex)" &&
                  !regex_patt.test($option.val()))
              ) {
                show = false;
              }
            ...
            if (show == true) {
              $("#" + unit_tag + " #" + condition.then_field).removeClass(
                "wpcf7cf-hidden"
              );
            }
          ...
          var checked_value_index = $.inArray(
            condition.if_value,
            checked_values
          );
          ...
          for (var ind = 0; ind < checked_values.length; ind++) {
            if (
              (condition.operator == "equals" &&
                checked_values[ind] == condition.if_value) ||
              (condition.operator == "not equals" &&
                checked_values[ind] != condition.if_value) ||
              (condition.operator == "equals (regex)" &&
                regex_patt.test(checked_values[ind])) ||
              (condition.operator == "not equals (regex)" &&
                !regex_patt.test(checked_values[ind])) ||
              (condition.operator == ">" &&
                checked_values[ind] > condition.if_value) ||
    ...
    for (var i = 0; i < options.length; i++) {
      ...
      $(
        ...
        "input paste change",
        { unit_tag: unit_tag, conditions: conditions, settings: settings },
        function(e) {
